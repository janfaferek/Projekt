...
void dodaj_rosline(Roslina roslina, Koszyk &koszyk)
{
    roslina.typ = TypRosliny::Warzywo;
    koszyk.push_back(roslina);
    roslina.typ = TypRosliny::Owoc;
    koszyk.push_back(roslina);
}
...
bool czy_jest_gruszka(const Koszyk &koszyk)
{
    auto it = std::find_if(
        ...
        koszyk.end(),
        [&](const Roslina &r) {
            return r.nazwa == "Gruszka";
        }
        );

    return it != koszyk.end();
}
...
void usun_zaczynajace_sie_od(Koszyk &koszyk, char litera, TypRosliny typ)
{
    auto it = std::remove_if(
        ...
        {
            char pierwsza = std::tolower(roslina.nazwa[0]);
            char lit = std::tolower(litera);

            return pierwsza == lit && roslina.typ == typ;
        }
        );
